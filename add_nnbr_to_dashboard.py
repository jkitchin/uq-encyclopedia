#!/usr/bin/env python
"""Add Neural Network BLR results to the dashboard."""
...
for nnbr_file in nnbr_files:
    ...
    '''
    nnbr_rows.append(row)

print(f"Generated {len(nnbr_rows)} table rows")
...
if dd_match:
    ...
    # Add NNBR rows to the existing content
    new_tbody = tbody_content + '\n' + '\n'.join(nnbr_rows) + '\n                    '
    new_table = table_start + '\n' + new_tbody + table_end

    # Replace in HTML
    html = html[:dd_match.start()] + new_table + html[dd_match.end():]
    print("✓ Added NNBR rows to data-driven table")
else:
    print("⚠️ Could not find data-driven table")

# Update the UQ Method filter to include NNBR
filter_method_pattern = r'(<select id="filterMethodDD".*?<option value="NNGMM">Neural Network GMM</option>)'

if re.search(filter_method_pattern, html, re.DOTALL):
    filter_replacement = r'\1\n                    <option value="NNBR">Neural Network BLR</option>'
    html = re.sub(filter_method_pattern, filter_replacement, html, flags=re.DOTALL)
    print("✓ Added NNBR to UQ Method filter")
else:
...
print(f"✓ Added {len(nnbr_rows)} Neural Network BLR results to data-driven tab")
print("\nRefresh your browser to see the NNBR results!")
