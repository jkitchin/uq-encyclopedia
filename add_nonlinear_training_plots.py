#!/usr/bin/env python
"""Add training data plots for each nonlinear dataset/noise combination to the dashboard."""
...
print("Generating training data plots for nonlinear dataset tab...")
...
DATASET_CONFIGS = {
    'ExponentialDecay': {
        ...
        'description': 'Exponential Decay: y = a·exp(-bx) + c'
    },
    'LogisticGrowth': {
        'class': LogisticGrowthDataset,
        'params': {'L': 1.0, 'k': 10.0, 'x0': 0.5},
        'description': 'Logistic Growth: y = L / (1 + exp(-k(x - x₀)))'
    },
    'MichaelisMenten': {
        'class': MichaelisMentenDataset,
        'params': {'Vmax': 1.0, 'Km': 0.3},
        'description': 'Michaelis-Menten: y = Vmax·x / (Km + x)'
    },
    'Gaussian': {
        'class': GaussianDataset,
        'params': {'a': 1.0, 'mu': 0.5, 'sigma': 0.15},
        'description': 'Gaussian: y = a·exp(-(x-μ)²/(2σ²))'
    }
}
...
print("\nGenerating HTML for nonlinear dataset tab...")
...
# Find the nonlinear tab and insert plots at the beginning
marker_start = '<div id="main-tab-nonlinear" class="main-tab-content">'

if marker_start in html_content:
    # Find the position right after the tab starts
    start_pos = html_content.find(marker_start) + len(marker_start)

    # Insert the plots section right after the tab opening
    plots_section = '\n\n                <h3>Training Data Visualizations</h3>\n' + all_plots_html + '\n\n            '
    html_content = html_content[:start_pos] + plots_section + html_content[start_pos:]
else:
    print("Warning: Could not find nonlinear tab marker")

# Save updated dashboard
with open(dashboard_path, 'w', encoding='utf-8') as f:
    f.write(html_content)

print(f"\n✓ Training data plots added to nonlinear tab in: {dashboard_path}")
