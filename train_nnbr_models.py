#!/usr/bin/env python
"""Train Neural Network BLR models on all datasets."""
...
print("=" * 70)
print("NEURAL NETWORK BLR UQ BENCHMARK")
print("=" * 70)

# Results directory
results_dir = Path('results/nnbr_fits')
...
for dataset_name, dataset_config in DATASET_CONFIGS.items():
    for noise_model in NOISE_MODELS:
        for noise_level in NOISE_LEVELS:
            ...
            # Create Bayesian Ridge regressor for uncertainty estimation
            br = BayesianRidge()

            # Create NNBR (Neural Network + Bayesian Linear Regression)
            nnbr = NeuralNetworkBLR(nn=mlp, br=br)
            ...
            print("  Fitting NNBR model with calibration...")
            nnbr.fit(X_train_fit, y_train_fit, val_X=X_train_cal, val_y=y_train_cal)

            # Make predictions on evaluation set
            y_pred, y_std = nnbr.predict(X_eval, return_std=True)

            # Flatten y_pred if needed (to avoid broadcasting issues)
            if y_pred.ndim > 1:
                y_pred = y_pred.flatten()

            # Compute 95% prediction intervals
            # For NNBR, we use 1.96 * std for 95% coverage
            z_score = 1.96
            ...
            # Save to JSON
            result_file = results_dir / f'{dataset_name}_{noise_model}_noise{int(noise_level*100):02d}_NNBR.json'
...
summary_df.to_csv(results_dir / 'nnbr_results_summary.csv', index=False)

print(f"\n✓ Completed training {len(all_results)} NNBR models")
