#!/usr/bin/env python
"""Generate interactive Plotly visualizations for NNBR fits - matching dashboard style."""
...
output_dir = Path('results/figures/nnbr_fits_html')
...
# Find all NNBR result files
result_files = sorted(nnbr_results_dir.glob('*_NNBR.json'))

print(f"Generating visualizations for {len(result_files)} NNBR fits (dashboard style)")
...
for result_file in result_files:
    ...
    title += f"UQ Method: NNBR (NN+BLR) | Coverage: {coverage:.1%} | RMSE: {rmse:.4f} | R²: {r2:.4f}"
    ...
    # Save to HTML
    output_file = output_dir / f'{dataset_name}_{noise_model}_noise{noise_level:02d}_NNBR.html'
