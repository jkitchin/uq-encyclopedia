#!/usr/bin/env python
"""Generate interactive Plotly visualizations for NNGMM fits - matching dashboard style."""
...
output_dir = Path('results/figures/nngmm_fits_html')
...
# Find all NNGMM result files
result_files = sorted(nngmm_results_dir.glob('*_NNGMM.json'))

print(f"Generating visualizations for {len(result_files)} NNGMM fits (dashboard style)")
...
for result_file in result_files:
    ...
    title += f"UQ Method: NNGMM (NN+GMM) | Coverage: {coverage:.1%} | RMSE: {rmse:.4f} | R²: {r2:.4f}"
    ...
    # Save to HTML
    output_file = output_dir / f'{dataset_name}_{noise_model}_noise{noise_level:02d}_NNGMM.html'
