#!/usr/bin/env python
"""Train Gaussian Process models with RBF + White Noise kernel on all datasets."""
...
print("=" * 70)
print("GAUSSIAN PROCESS UQ BENCHMARK")
print("=" * 70)

# Results directory
results_dir = Path('results/gp_fits')
...
for dataset_name, dataset_config in DATASET_CONFIGS.items():
    for noise_model in NOISE_MODELS:
        for noise_level in NOISE_LEVELS:
            ...
            noise_variance = (noise_level) ** 2 * np.var(y_train)
            if noise_variance < 1e-6:
                noise_variance = 0.01 * np.var(y_train)

            # Define kernel with bounds for hyperparameter optimization
            kernel = (
                C(1.0, (1e-3, 1e3)) * RBF(length_scale=0.1, length_scale_bounds=(1e-3, 1.0)) +
                WhiteKernel(noise_level=noise_variance, noise_level_bounds=(1e-5, 1e1))
            )
            ...
            gp = GaussianProcessRegressor(
                ...
                normalize_y=True,  # Normalize y for better numerical stability
                random_state=42
            )
            ...
            # For GP, we use 1.96 * std for 95% coverage
            z_score = 1.96
            ...
            # Save to JSON
            result_file = results_dir / f'{dataset_name}_{noise_model}_noise{int(noise_level*100):02d}_GP.json'
...
summary_df.to_csv(results_dir / 'gp_results_summary.csv', index=False)

print(f"\n✓ Completed training {len(all_results)} GP models")
